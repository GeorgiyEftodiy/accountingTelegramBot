...
async def general(message):
    await bot.send_message(message.from_user.id,'📍 Руководство по пользованию:\n'
                                                ...
                                                '🔹 📈 Создать отчет - В конце месяца, чтобы получить деньги за вашу работу, вам необходимо создать '
                                                'отчет вашей работы по нажатию данной кнопки и отправить его в группу с отчетами. Чтобы очистить свою '
                                                'историю работ, напишите команду "/очистка" боту и можете начинать новый месяц с чистого листа')
...
async def adduser(message):
    if (botDB.user_exist(message.from_user.id)):
        botDB.clear_history(message.from_user.id)
    ...
    if (botDB.user_exist(message.from_user.id)):
        if message.chat.type == 'private':
            ...
            if '@zavodrezbot Тип(100) Количество=' in message.text:
                ...
                botDB.add_work(message.from_user.id, message.from_user.first_name, '100-тки', amount, summ)
            ...
            if '@zavodrezbot Тип(50) Количество=' in message.text:
                ...
                botDB.add_work(message.from_user.id, message.from_user.first_name, '50-тки', amount, summ)
